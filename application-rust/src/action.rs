...
/// A common API for returning [`Task`]s and/or generic
...
pub struct Action<I, Message> {
    /// Instructions mutate state and have no affect on the runtime.
    pub instruction: Option<I>,
    /// Tasks use the runtime and can mutate state after the fact.
    pub task: Task<Message>,
}

impl<I, Message> Action<I, Message> {
    /// Create a new `Action` with no `Instruction` or [`Task`].
    pub fn none() -> Self {
        Self {
            instruction: None,
            task: Task::none(),
    ...
    /// Create a new `Action` with an `Instruction`.
    ...
    /// Map the message of the `Action`'s [`Task`] to a different type.
